...
export default function TransactionDetailForm() {
    ...
    // ── Category options (default + any custom from API) ──────────────────────
    const defaultCategoryOptions = ['Piece', 'Pound'];
    ...
    useEffect(() => {
        ...
        fetch(`/transactions?project_id=${selectedProjectId}`)
            ...
            .then(data => {
                ...
                const usedCategories = filtered
                    .map(tx => tx.category)
                    .filter(c => !defaultCategoryOptions.includes(c));
                const uniqueCustomCats = Array.from(new Set(usedCategories));
                setCategoryOptions([...defaultCategoryOptions, ...uniqueCustomCats]);
                            ...
                            <tr>
                                {[
                                    ...
                                    <th
                                        ...
                                        style={{
                                            padding: '0.5rem',
                                            // make Entry much wider:
                                            minWidth: h === 'Entry'
                                                ? '200px'
                                                : ['Total', 'Discount', 'Net', 'Category', 'Product','Price Cat'].includes(h)
                                                    ? '135px'
                                                    : '80px'
                                        }}
                                    >
